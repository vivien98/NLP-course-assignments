...
def naiveSoftmaxLossAndGradient(
    ...
    preSoftMax = np.matmul(outsideVectors,centerWordVec)
    SoftMax = softmax(preSoftMax)
    loss = -np.log(SoftMax[outsideWordIdx])
    (numVocab, wordVecLen) = np.shape(outsideVectors)
    centerWordVec = np.reshape(centerWordVec,(wordVecLen,1))    
    outsideGradMat = np.reshape(SoftMax,(numVocab,1))
    outsideGradMat[outsideWordIdx,0] = -(1-SoftMax[outsideWordIdx])
    
    gradCenterVec = np.reshape(np.matmul(np.transpose(outsideGradMat), outsideVectors),(wordVecLen,))

    
    gradOutsideVecs = np.matmul(outsideGradMat, np.transpose(centerWordVec))
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    outsideVecMatSampled = np.take(outsideVectors, indices, axis = 0)
    (numVocab, wordVecLen) = np.shape(outsideVectors)
    sigmoidVec = np.transpose(sigmoid(np.matmul(outsideVecMatSampled,centerWordVec)))
    sigmoidVecComp = 1 - sigmoidVec
    sigmoidVecComp[0] = sigmoidVec[0]
    loss = -np.sum(np.log(sigmoidVecComp))

    sigmoidVec[0] = -1+ sigmoidVec[0]
    gradCenterVec = np.matmul(sigmoidVec,outsideVecMatSampled)

    sigmoidSumVec = np.zeros((numVocab,1))
    for i in range(K+1):
        sigmoidSumVec[indices[i],0] += sigmoidVec[i]

    gradOutsideVecs = np.matmul(sigmoidSumVec,np.reshape(centerWordVec,(1,wordVecLen)))
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        lossTemp, gradCenterTemp, gradOutsideTemp = word2vecLossAndGradient(centerWordVec, word2Ind[word],outsideVectors,dataset)
        loss += lossTemp
        gradCenterVecs[word2Ind[currentCenterWord]] += gradCenterTemp
        gradOutsideVectors += gradOutsideTemp
    ### END YOUR CODE
