...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        for i in range(len(minibatch_parse)):
            minibatch_parse[i].parse_step(transitions[i])
            if len(minibatch_parse[i].stack) == 1 and len(minibatch_parse[i].buffer) == 0:
                unfinished_parses = unfinished_parses[0:i]+unfinished_parses[i+1:]
