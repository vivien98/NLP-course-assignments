...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(torch.tensor(np.zeros((self.embed_size*self.n_features,self.hidden_size)))).float())
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(torch.tensor(np.zeros((1,self.hidden_size)))).float())
        self.hidden_to_logit_weights = nn.Parameter(nn.init.xavier_uniform_(torch.tensor(np.zeros((self.hidden_size,self.n_classes)))).float())
        self.hidden_to_logit_bias = nn.Parameter(nn.init.uniform_(torch.tensor(np.zeros((1, self.n_classes)))).float())
    ...
    def embedding_lookup(self, w):
        ...
        final_mat = []
        for row in w:
            final_mat.append(list(torch.flatten(torch.index_select(self.embeddings,0,row))))
        x = torch.tensor(final_mat)
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        logits = torch.matmul(h,self.hidden_to_logit_weights) + self.hidden_to_logit_bias
        ### END YOUR CODE
        return logits
