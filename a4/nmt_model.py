...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens1, (last_hidden,last_cell) = self.encoder(nn.utils.rnn.pack_padded_sequence(X,source_lengths))
        (enc_hiddens2, dump) = pad_packed_sequence(enc_hiddens1)
        enc_hiddens = torch.transpose(enc_hiddens2,0,1)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y,1):
            ...
            dec_state, o_t, dump = self.step(Ybar_t,dec_state,enc_hiddens,enc_hidden_proj,enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.squeeze(torch.bmm(torch.unsqueeze(dec_hidden,1),torch.transpose(enc_hiddens_proj,1,2)),dim=1)
