...
if __name__ == '__main__':
    ...
    block_size = 128
    text = open(args.pretrain_corpus_path,encoding="utf-8",errors="ignore").read()
    ...
    if args.variant == 'vanilla':
        ...
        model = model.GPT(mconf)
        model = model.cuda()# TODO [part g]: Make some other model here
    ...
    if args.function == 'pretrain':
        ...
        #     num_workers=4
        pretrain_text = open(args.pretrain_corpus_path,'r',errors="ignore").read()
        pretrain_dataset = dataset.CharCorruptionDataset(pretrain_text,block_size)
        pre_tconf = trainer.TrainerConfig(max_epochs=650,batch_size=128,learning_rate=6e-3,lr_decay=True,warmup_tokens=512*20,final_tokens=200*len(pretrain_dataset)*block_size,ckpt_path=args.writing_params_path,num_workers=4)
        pre_Trainer = trainer.Trainer(model,pretrain_dataset,None,pre_tconf)
        pre_Trainer.train()
        ...
        finetune_text = open(args.finetune_corpus_path,'r',errors="ignore").read()
        finetune_dataset = dataset.NameDataset(pretrain_dataset,finetune_text)
        tconf = trainer.TrainerConfig(max_epochs=75,batch_size=128,learning_rate=6e-4,lr_decay=True,warmup_tokens=512*20,final_tokens=200*len(pretrain_dataset)*block_size,ckpt_path=args.writing_params_path,num_workers=4)
        ...
        with open(args.outputs_path, 'w') as fout:
            predictions = []
            for line in tqdm(open(args.eval_corpus_path,encoding="utf-8",errors="ignore")):
                x = line.split('\t')[0]
                x = x + '⁇'
