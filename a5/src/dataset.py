...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(document) < 4:
            document=document+" "*(5-len(document))

        truncate_to_len = random.randint(4,int(self.block_size*7/8))
        document = document[0:truncate_to_len+1]
        doc_len = len(document)
        mask_len = random.randint(1,int(doc_len/2))
        start_idx = random.randint(1,doc_len-mask_len)
        prefix = document[0:start_idx]
        mask = document[start_idx:start_idx+mask_len]
        suffix = document[start_idx+mask_len:]

        final_str = prefix+self.MASK_CHAR+suffix+self.MASK_CHAR+mask
        padding = self.PAD_CHAR*(self.block_size-len(final_str))
        final_str = final_str+padding

        x_char = final_str[:-1]
        y_char = final_str[1:]

        x = torch.LongTensor([self.stoi[char] for char in x_char])
        y = torch.LongTensor([self.stoi[char] for char in y_char])
...
if __name__ == '__main__':
    ...
    if args.dataset_type == 'namedata':
        ...
        name_dataset = NameDataset(corruption_dataset,
            open('birth_places_train.tsv',errors="ignore").read())
